...
abstract class BaseObject extends Collection
{
    ...
    public function mapRelatives()
    {
        ...
        return $this->items = $collection->map(function ($value, $key) use ($relations) {
            if (!$relations->offsetExists($key)) {
                return $value;
            }
            $className = $relations->offsetGet($key);
    ...
    {
        return !empty($data['result']) ? $data['result'] : $data;
    }
    /**
