...
class BotHandler implements RequestHandlerInterface
{
    ...
    public function __construct($config, string $containerName)
    {
        $this->config = $config;
    ...
    {
        $telegram = new Api($this->config['telegram']['api_key']);
        ...
        $keyboard = [["Последние статьи"], ["Мотивация"]]; //Клавиатура
        if($text){
            if ($text == "/start") {
            ...
            } elseif ($text == "Мотивация") {
                $list = ['Это ерунда', 'Плохо!', 'Руки выпрями!', 'У меня 6 летние дети делают лучше', 'Это что за стойка такая?'];
                shuffle($list);
                $telegram->sendMessage([ 'chat_id' => $chat_id, 'text' => $list[0] ]);
